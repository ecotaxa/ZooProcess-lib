...
class ConnectedComponentsSegmenter:
    ...
    @staticmethod
    def get_regions_using_floodfill(labels:ndarray, cc_id:int, x, y, w, h, width, height):
        if x == 0 or y == 0 or x + w == width or y + h == height:
            # The shape touches an image border
            sub_labels = cropnp(
                image=labels, top=y, left=x, bottom=y + h, right=x + w
    ...
    def get_regions_using_contours(
        ...
        # Above 'holes' is not pixel-exact as the edges b/w object and holes is drawn, eliminate them
        holes ^= obj_mask
        sub_mask = obj_mask
        return holes, sub_mask
