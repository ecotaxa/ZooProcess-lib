...
class Border:
    ...
    def left_limit(self) -> int:
        ...
        self.draw_image = draw_box(self.draw_image, x=k, y=self.height / 2 - self.height / 8, h=self.height * 0.25,
                                   w=self.step, color=(255, 0, 0))

        greyvalb = int(mean * self.greytaux)
        print(f"greyvalb: {greyvalb}")

        while k > 0:
            img = crophw(self.image, top=k, left=self.height / 2 - self.height / 8, width=self.height / 4,
                         height=self.step)
            print(f"shape of cropped image: {img.shape}")
            print(
            ...
            mean = np.mean(img, axis=None)
            # saveimage(img, self.name, "crop_left" + "_" + str(int(k)) + "_" + str(int(mean)), ext="tiff", path=self.crop_output_path)
            self.draw_image = draw_box(self.draw_image, x=k, y=self.height / 2 - self.height * 0.125,
                                       h=self.height * 0.25, w=self.step, color=(255, 0, 0))
    ...
    def right_limit(self) -> int:
        ...
        self.draw_image = draw_box(self.draw_image, x=k, y=self.height / 4, h=self.height / 4, w=10 * self.step,
                                   color=(255, 255, 0))
        ...
        while k <= self.width:
            ...
            mean = np.mean(img, axis=None)
            # saveimage(img, self.name, "crop_right" + "_" + str(int(k)) + "_" + str(int(mean)), ext="tiff", path=self.crop_output_path)
            self.draw_image = draw_box(self.draw_image, x=k, y=self.height / 4, h=self.height / 4, w=self.step,
                                       color=(255, 255, 0))
            # print(f"Mean of cropped image: {mean}")
            if mean < greyvalb:
                limit = k
    ...
    def bottom_limit(self):
        ...
        self.draw_image = draw_box(self.draw_image, x=self.width / 6, y=k, w=self.width * 0.15, h=self.step,
                                   color=(255, 0, 0))
    ...
    def top_limit(self) -> int:
        ...
        self.draw_image = draw_box(self.draw_image, y=k, x=self.width / 2 - self.width / 8, w=self.width / 4,
                                   h=self.step, color=(255, 0, 0))

        greyvalb = int(mean * self.greytaux)

        while k > 0:
            # img = crophw(self.image, left=self.width / 2 - self.width/4, top=k, width=self.width * 0.2, height=self.step)
            img = crophw(self.image, top=self.width / 2 - self.width / 4, left=k, height=self.width * 0.2,
                         width=self.step)
