...
class Calculater:
    """
    Straight from https://github.com/matthiasnwt/feret
    """
    ...
    def __init__(self, img, edge):
        ...
        self.hull = self.find_convexhull()
    ...
    def find_convexhull(self):
        ...
        if self.edge:
            ...
            return (
                cv.convexHull(np.array([new_ys, new_xs]).T)
                .T.reshape(2, -1)
                .astype(float)
            )
        else:
            return (
                cv.convexHull(np.transpose(np.nonzero(self.img)))
                .T.reshape(2, -1)
                .astype(float)
            )
    ...
    #         if angle == np.pi / 2:
    #             plt.axhline(
    #                 coord1[0], linestyle="--", color=color, label=name + " Baseline"
    #             )
    #             plt.axhline(coord2[0], linestyle="--", color=color)
    #             plt.axvline(coord1[1], color=color, label=name + " Line")
    #         elif angle == 0:
    #             plt.axvline(
    #                 coord1[1], linestyle="--", color=color, label=name + " Baseline"
    #             )
    ...
    #
    #             plt.plot(
    #                 xs, base_ys, linestyle="--", color=color, label=name + " Baseline"
    #             )
    ...
    #     draw_lines(
    #         "MaxFeret90",
    #         self.maxf90_coords[0],
    #         self.maxf90_coords[1],
    #         "green",
    #         self.maxf90_angle,
    #         self.maxf90_t,
    #         self.maxf90_coords,
    ...
    #     draw_lines(
    #         "MinFeret90",
    #         self.minf90_coords[0],
    #         self.minf90_coords[1],
    #         "orange",
    #         self.minf90_angle,
    #         self.minf90_t,
    #         self.minf90_coords,
    ...
    @staticmethod
    def cross2d(x: np.ndarray, y: np.ndarray):
        """ https://github.com/numpy/numpy/issues/26620 """
    ...
    def calculate_minferet(self):
        ...
        for i in range(length):
            ...
            ds = np.abs(self.cross2d(p2 - p1, p1 - self.hull.T) / norm(p2 - p1))
        ...
        if self.edge:
            self.minf /= 2.0
            self.minf_coords /= 2.0

        if x0 == x1:
            self.minf_t = self.minf_coords.T[0][2]
        else:
            self.minf_t = (
                self.minf_coords.T[0][2]
                - np.tan(self.minf_angle) * self.minf_coords.T[1][2]
            )
    ...
    def calculate_maxferet(self):
        ...
        self.maxf_coords = self.hull.T[
            np.array((maxf_coords_index_x, maxf_coords_index_y))
        ]
        ...
        if x1 == x0:
            ...
            self.maxf_t = (
                self.maxf_coords.T[0][1]
                - np.tan(self.maxf_angle) * self.maxf_coords.T[1][1]
            )
    ...
    def calculate_maxferet90(self):
        ...
        self.maxf90, self.maxf90_coords = self.calculate_distances(
            self.maxf90_angle - np.pi / 2
        )
        ...
        if self.maxf90_angle == 0 or self.maxf90_angle == np.pi:
            ...
            self.maxf90_t = (
                self.maxf90_coords.T[0][1]
                - np.tan(self.maxf90_angle) * self.maxf90_coords.T[1][1]
            )
    ...
    def calculate_minferet90(self):
        ...
        self.minf90, self.minf90_coords = self.calculate_distances(
            self.minf90_angle - np.pi / 2
        )
        ...
        if self.minf90_angle == 0 or self.minf90_angle == np.pi:
            ...
            self.minf90_t = (
                self.minf90_coords.T[0][1]
                - np.tan(self.minf90_angle) * self.minf90_coords.T[1][1]
            )
