...
class Converter(object):
    ...
    def do_file_to_file(self, from_file: Path, to_file: Path, is_a_background: bool):
        """
        ...
        :param is_a_background: self-explaining
        """
        converted_image, resolution = self.do_file_to_image(from_file, is_a_background)

        dest_file = to_file.as_posix()
        saveimage(converted_image, dest_file, dpi=(resolution, resolution))

    def do_file_to_image(
        self, from_file: Path, is_a_background: bool
    ) -> Tuple[ndarray, int]:
