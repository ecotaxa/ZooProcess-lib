...
@pytest.mark.skip(reason="Skipping this test for now because of XYZ reason.")
def test_resize_background_but_(self, TP):
    back_name = "20240112_1518_back_large_1.tif"
    back_file = Path(TP.back, back_name)
    print(f"file: {back_file}")
    back_image = loadimage(back_file.as_posix())
...
def test_all(self, TP):
    ...
    scan_unbordered = cropnp(image, left=scan_limit_left, top=scan_limit_top, right=scan_limit_right,
...
def test_subtracted_to8bit(self, TP):
    ...
    image_back_8bit = convertion(image_substracted, sample, TP=TP)
...
def test_saturation(self, TP):
    ...
    data2_rgb[sat_mask] = np.array([255, 0, 0],
                                   np.uint8)  # [255, 0, 0] applies red color, in case the image is uint16, use [65535, 0, 0].
