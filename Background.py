...
class Background:
    ...
    def __init__(self, image, image_name, resolution = 300, scan_resolution = 2400, output_path = None ):
        self.image = image
        ...
        if image_name.find("large"): self.frametypeback = "large"
        if image_name.find("narrow"): self.frametypeback = "narrow"
    ...
    def redim(self):
        ...
        image_resized = cv2.resize( image_median, (self.L, self.H), fx, fy, interpolation)

        saveimage( image_resized, self.name, "resized" , ext="tiff", path=self.output_path)
